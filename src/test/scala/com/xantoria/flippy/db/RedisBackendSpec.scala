...
class RedisBackendSpec extends BaseSpec with DockerRedis with DockerTestKit {
  ...
  private val serEngine = SerializationEngine()
  private implicit val formats = DefaultFormats + serEngine
  ...
  "The redis backend" should "allow setting and retrieving switch conditions" in {
    ...
    val backend = new RedisBackend("localhost", 6379, "testtesttest")
