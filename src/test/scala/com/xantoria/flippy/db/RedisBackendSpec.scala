...
class RedisBackendSpec extends BaseSpec with DockerRedis with DockerTestKit {
  ...
  "The redis backend" should "allow setting and retrieving switch conditions" in {
    ...
    val backends = Set(
      "DB" -> new RedisBackend("localhost", redisPort, "flippy:test:backend:setget"),
      "Hash" -> new RedisHashBackend("localhost", redisPort, "flippy:test:backend:setget:hash")
    )
    val cond = Condition.Equals("Ms. Cloud") on "name"

    backends foreach {
      case (backendType, backend) => withClue(s"Testing $backendType backend:") {
  ...
  it should "allow listing keys" in {
    val backends = Set(
      "DB" -> new RedisBackend("localhost", redisPort, "flippy:test:backend:list"),
      "Hash" -> new RedisHashBackend("localhost", redisPort, "flippy:test:backend:list:hash")
    )
